...
def test_authentication():
            ...
            for test_idx, test_path in enumerate(test_fingerprints_paths):
                ...
                for collection_dataset_idx, dataset_collection in enumerate(fingerprints_templates_collections):
                    ...
                    for collection_idx, collection in enumerate(dataset_collection):
                        ...
                        truth_table[dataset_idx, table_idx, collection_idx] = (collection_idx == fingerprint_idx)
                        result_table[dataset_idx, table_idx, collection_idx] = is_good

    all_true_positive_num = 0
    all_true_negative_num = 0
    all_false_positive_num = 0
    all_false_negative_num = 0
    for dataset_idx in range(DATATSETS_NUM):
        dataset_truth_table = truth_table[dataset_idx].flatten()
        dataset_result_table = result_table[dataset_idx].flatten()

        # print("Classification Stats:")
        # print(skl.metrics.classification_report(dataset_truth_table, dataset_result_table, labels=[False, True], digits=4, zero_division=0.0))

        true_false_table = dataset_truth_table == dataset_result_table
        true_mask = np.argwhere(true_false_table)

        true_table = dataset_result_table[true_mask]
        true_positive_num = np.count_nonzero(true_table)
        true_negative_num = len(true_table) - true_positive_num

        false_positive_num = np.count_nonzero(dataset_result_table) - true_positive_num
        false_negative_num = len(dataset_result_table) - false_positive_num - true_positive_num - true_negative_num

        all_true_positive_num += true_positive_num
        all_true_negative_num += true_negative_num
        all_false_positive_num += false_positive_num
        all_false_negative_num += false_negative_num

        accuracy = 0.0 if len(dataset_result_table) == 0 else (true_positive_num + true_negative_num) / len(dataset_result_table)
    ...
    accuracy = 0.0 if len(result_table) == 0 else (all_true_positive_num + all_true_negative_num) / len(result_table)

    print()
    print("All Datasets")
    print(f"Results num: {len(result_table)}")
    print(f"True Positive: {all_true_positive_num}, expected: {DATATSETS_NUM * FINGERPRINTS_NUM * TEST_IMG_PER_FINGERPRINT}")
    print(f"True Negative: {all_true_negative_num}, expected: {(FINGERPRINTS_NUM ** 2 - FINGERPRINTS_NUM) * TEST_IMG_PER_FINGERPRINT * DATATSETS_NUM}")
    print(f"False Positive: {all_false_positive_num}, expected: 0")
    print(f"False Negative: {all_false_negative_num}, expected: 0")
    print(f"False Accept Rate (FAR): {all_false_positive_num / (all_false_positive_num + all_true_negative_num)}")
    print(f"False Reject Rate (FRR): {all_false_negative_num / (all_false_negative_num + all_true_positive_num)}")
    print(f"Genuine Accept Rate (GAR): {all_true_positive_num / (all_true_positive_num + all_false_negative_num)}")
    print(f"Accuracy: {accuracy * 100.0} %")
