...
binarized, skeleton, contour, orientation_field, reliability_map, core_point, delta_point, minutae = fpa.get_fingerprint_data(fingerprint, 16, 5, 5, 0.5, 0.4 * np.pi)

endings_mask = [m['pos'] for m in minutae if m['type'] == fpa.MINUTIAE_ENDING] + core_point
bifurcation_mask = [m['pos'] for m in minutae if m['type'] == fpa.MINUTIAE_BIFURCATION] + core_point
...
plt.scatter(np.array(endings_mask)[..., 1], np.array(endings_mask)[..., 0], color='green', label='Ending', s=10, marker=markers.TICKLEFT)
plt.scatter(np.array(bifurcation_mask)[..., 1], np.array(bifurcation_mask)[..., 0], color='violet', label='Bifurcation', s=10,  marker='1')
plt.legend()
