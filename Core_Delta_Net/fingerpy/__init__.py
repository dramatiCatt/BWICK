...
class Fingerprint():
    ...
    def _preprocess(self) -> None:
        ...
        self._crop_start_y, self._crop_end_y, self._crop_start_x, self._crop_end_x = ctour.get_reliable_region_rectangle(self._contour)

        # MOVE CONTOUR
        self._contour[..., 0] -= self._crop_start_x
        self._contour[..., 1] -= self._crop_start_y

        # CROP IMAGES
        self._normalized = self._normalized[self._crop_start_y:self._crop_end_y, self._crop_start_x:self._crop_end_x]
        self._orientation_field = self._orientation_field[self._crop_start_y:self._crop_end_y, self._crop_start_x:self._crop_end_x]
        self._reliability_map = self._reliability_map[self._crop_start_y:self._crop_end_y, self._crop_start_x:self._crop_end_x]
...
@timer
def authenticate(fingerprint_minutiae: list[minuti.Minutiae], fingerprint_templates: list[FingerprintTemplate], auth_threashold: float) -> bool:
    best_match = 0
